...
export const DriverApp: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-[360px] mx-auto bg-neutral-950 border-[6px] border-neutral-800 rounded-[36px] overflow-hidden shadow-2xl flex flex-col h-[680px] relative text-white select-none">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-neutral-900 text-neutral-100 text-xs relative">
        ...
        <div className="bg-gradient-to-br from-neutral-800/90 to-neutral-900 border border-neutral-700/80 rounded-2xl p-3.5 space-y-2.5">
          <div className="flex items-center justify-between text-neutral-400 text-[11px]">
            ...
            <span className="text-[10px] text-emerald-400 font-semibold">Atualização em tempo real</span>
          </div>
          ...
          <div className="bg-neutral-800/90 border border-neutral-700 rounded-2xl p-3.5 space-y-3 shadow-md">
            ...
            <div className="space-y-2 pt-1">
              ...
              {activeTrip.status === 'in_progress' && (
                <button
                  onClick={finishTrip}
                  className="w-full py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs flex items-center justify-center gap-1.5 transition-all"
                >
                  <CheckCircle2 className="w-4 h-4" />
                  <span>Finalizar Corrida & Creditar Carteira</span>
                </button>
              )}
            </div>
          </div>
        ...
        {/* ADAPTIVE GPS SIMULATOR CONTROLS (Chapter 5) */}
        <div className="bg-neutral-800/80 border border-neutral-700/80 rounded-2xl p-3 space-y-2.5">
          ...
          <div className="bg-neutral-900/90 rounded-xl p-2 text-[10px] space-y-1 text-neutral-400 border border-neutral-800">
            <div className="flex justify-between">
              ...
              <span className="text-white font-semibold">{gpsRule.label.split('(')[0]}</span>
            </div>
            <div className="flex justify-between">
              <span>Intervalo:</span>
              <span className="font-mono text-[#FFC107] font-bold">{gpsRule.intervalSec} segundos</span>
            </div>
          </div>
        </div>

        {/* SYSTEM STATUS CARD */}
        <div className="bg-neutral-800/50 border border-neutral-700/60 rounded-xl p-2.5 text-[10px] text-neutral-400 space-y-1">
          <div className="flex justify-between">
            <span>Geohash Atual:</span>
            <span className="font-mono text-neutral-200">{currentDriver.geohash}</span>
          </div>
          <div className="flex justify-between">
            <span>Sincronização Firestore:</span>
            <span className="text-emerald-400 font-semibold">Ativa (&lt;150ms)</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-3 bg-neutral-950 border-t border-neutral-800 shrink-0 text-center text-[10px] text-neutral-500">
        Go.Bro.Aao Motorista v1.0 • Luanda, Angola
      </div>
    </div>
  );
};
